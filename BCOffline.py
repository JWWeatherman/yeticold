...
def BCdisplaytransaction():
    ...
    if request.method == 'GET':
        ...
        response = subprocess.Popen(['~/yeticold/bitcoin/bin/bitcoin-cli -rpcwallet= signrawtransactionwithwallet '+transonehex+' \'[{"txid":"'+selectedutxo['txid']+'","vout":"'+selectedutxo['vout']+'","scriptPubKey":"'+selectedutxo['scriptPubKey']+'"}]\''],shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
        print(response)
