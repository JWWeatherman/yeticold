...
if len(sys.argv) == 1 or sys.argv[1].lower() not in [x.lower() for x in VALIDMODES]:
    ...
    if not os.path.exists(HOME + "/yeticold/bitcoin"):
        ...
        subprocess.run('sudo apt-get update && yes | sudo apt install python3-venv python3-pip sshpass libzbar0 tor', shell=True, check=False)
        subprocess.run('sudo apt-get update && sudo unattended-upgrade', shell=True, check=False)
