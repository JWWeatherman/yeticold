...
def scanDescriptor(request, currentroute, nextroute, offline=True):
    if request.method == 'POST':
        ...
        if response[1] != b'':
            ...
            return redirect(currentroute)
        if offline:
            handleResponse('~/yeticold/bitcoin/bin/bitcoin-cli -rpcwallet=yetiwalletpriv importdescriptors \'[{ "desc": "'+v.pubdesc+'", "timestamp": "now", "internal": true}]\'')
            ...
            handleResponse('~/yeticold/bitcoin/bin/bitcoin-cli -rpcwallet=yetiwalletpub importdescriptors \'[{ "desc": "'+v.pubdesc+'", "timestamp": "now", "internal": true}]\'')
...
def getSeeds(request, nextroute):
    if request.method == 'POST':
        ...
        handleResponse('~/yeticold/bitcoin/bin/bitcoin-cli -rpcwallet=yetiwalletpriv importdescriptors \'[{ "desc": "'+desc+'", "timestamp": "now", "internal": true}]\'')
...
def importSeeds(request, currentroute, nextroute):
    ...
    if request.method == 'POST':
        ...
        if (v.privkeycount >= 3):
            ...
            handleResponse('~/yeticold/bitcoin/bin/bitcoin-cli -rpcwallet=yetiwalletpriv importdescriptors \'[{ "desc": '+desc+'#'+ checksum +'", "timestamp": "now", "internal": true}]\'')
