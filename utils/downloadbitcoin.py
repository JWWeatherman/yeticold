...
if not os.path.exists(home + "/yeticold/bitcoin"):
	subprocess.call(['wget https://bitcoincore.org/bin/bitcoin-core-0.21.0/test.rc3/bitcoin-0.21.0rc3-x86_64-linux-gnu.tar.gz -P ~/yeticold/'],shell=True)
	subprocess.call(['rm ~/yeticold/sigcorrect 2> /dev/null'],shell=True)
	if not (os.path.exists(home + "/yeticold/SHA256SUMS.asc")):
		subprocess.call(['sudo wget https://bitcoincore.org/bin/bitcoin-core-0.21.0/test.rc3/SHA256SUMS.asc -P ~/yeticold/'],shell=True)
	...
	subprocess.call(['tar xvzf ~/yeticold/bitcoin-0.21.0rc3-x86_64-linux-gnu.tar.gz -C ~/yeticold'],shell=True)
	subprocess.call(['mv ~/yeticold/bitcoin-0.21.0rc3 ~/yeticold/bitcoin'],shell=True)
